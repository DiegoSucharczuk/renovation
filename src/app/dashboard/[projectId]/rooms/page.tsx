...
export default function RoomsPage() {
  ...
  const fetchData = async () => {
    try {
        ...
        taskCategories.forEach(category => {
          ...
          if (task && task.status !== 'NOT_RELEVANT') {
            roomTasks[category.name] = {
              ...
              autoUpdateStatus: (task as any).autoUpdateStatus !== undefined ? (task as any).autoUpdateStatus : true,
            };
          }
        });
        return {
      ...
      roomsWithTasks.sort((a, b) => a.name.localeCompare(b.name, 'he'));
      ...
      if (hiddenTask) {
        if (window.confirm('יש משימה מוסתרת בחדר זה. האם להחזיר אותה?')) {
          setTaskFormData({
            ...
            status: 'NOT_STARTED',
            progress: 0,
            startDate: hiddenTask.startDate || '',
            endDate: hiddenTask.dueDate || '',
  ...
  const handleSaveTask = async () => {
    ...
    // Validation: if status is manually set to DONE, progress must be 100%
    if (!taskFormData.autoUpdateStatus && taskFormData.status === 'DONE' && taskFormData.progress < 100) {
    ...
    // אם עדכון אוטומטי מופעל - חשב סטטוס לפי התקדמות ותאריכים
    // אבל אם הסטטוס הוא WAITING (חסום) - לא לשנות אותו
    if (taskFormData.autoUpdateStatus && taskFormData.status !== 'WAITING') {
      ...
      if (taskFormData.progress >= 100) {
        ...
        // אם יש התקדמות - זה בביצוע
        finalStatus = 'IN_PROGRESS';
      } else if (startDate && today >= startDate) {
        // אם הגיע תאריך ההתחלה אבל אין התקדמות - עדיין בביצוע
        finalStatus = 'IN_PROGRESS';
      } else {
        // אחרת - לא התחיל
    ...
    try {
      ...
      if (existingTask) {
        ...
        await updateDoc(doc(db, 'tasks', existingTask.id), {
          ...
          autoUpdateStatus: taskFormData.autoUpdateStatus,
        });
      } else {
        // צור משימה חדשה
        await addDoc(collection(db, 'tasks'), {
          ...
          title: taskFormData.description || editingTask.taskTypeName,
        ...
        <Dialog open={openTaskDialog} onClose={handleCloseTaskDialog} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box display="flex" alignItems="center" gap={1} sx={{ bgcolor: '#f5f5f5', p: 2, borderRadius: 1 }}>
                <input
                  type="checkbox"
                  id="autoUpdateStatus"
                  checked={taskFormData.autoUpdateStatus}
                  onChange={(e) => setTaskFormData({ ...taskFormData, autoUpdateStatus: e.target.checked })}
                  style={{ width: 20, height: 20, cursor: 'pointer' }}
                />
                <label htmlFor="autoUpdateStatus" style={{ cursor: 'pointer', userSelect: 'none' }}>
                  <Typography variant="body2">
                    עדכון אוטומטי של סטטוס לפי תאריכים והתקדמות
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    הסטטוס יעודכן אוטומטית: לא התחיל → בביצוע → הושלם
                  </Typography>
                </label>
              </Box>
            </Box>
